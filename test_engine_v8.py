...
class Engine:
    ...
    def _recurring_templates(self, user, request_date, home, profile):
        ...
        # High-frequency category-level interval templates
        # For any category with very consistent short-cycle patterns (7-21 day gaps, MAD <= 2)
        # Only add ONE interval template per category (most recent) to avoid over-counting
        ...
        for (direction, cat), g in hist[hist.direction == 'debit'].groupby(['direction', 'category']):
            ...
            if len(g) < 10:
                continue
            # Use recent events (last 90 days) for interval estimation
