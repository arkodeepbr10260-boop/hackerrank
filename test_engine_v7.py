...
class Engine:
        ...
        for uid, g in self.messages.groupby('user_id'):
            ...
            for _, m in g.sort_values('sent_at').iterrows():
                ...
                for s in re.findall(r'\b(20\d{2}[-/]\d{1,2}[-/]\d{1,2})\b', text):
                    try:
                        dates.append(pd.Timestamp(s.replace('/', '-')))
                    except Exception:
                        pass
                rule = {'text': text, 'sent': sent, 'dates': dates, 'nums': nums}
                if any(k in low for k in ['contract has ended', 'contract ended', 'no renewal',
                                           'no off-season income', 'termination', 'final employer payroll',
                                           'final salary', 'last salary']):
    ...
    def _recurring_templates(self, user, request_date, home, profile):
        ...
        # 1. Recurring Debits - strict monthly range 25-35 days with low variance
        ...
        for (direction, cat, desc), g in hist[hist.direction == 'debit'].groupby(['direction', 'category', 'description'], dropna=False):
            ...
            # Strict monthly range only
            if not (25 <= med <= 35):
                continue
            # Variance filter: gaps should be consistent (MAD <= 6 days)
            if float((gaps - med).abs().median()) > 6:
                continue
            last = g.iloc[-1]
        ...
        # High-frequency interval expenses: any category with very consistent short cycles
        # regardless of profile protection (consistency implies genuine recurring expense)
        used_cats_any = {t['category'] for t in templates if t['direction'] == 'debit'}
        for (direction, cat), g in hist[hist.direction == 'debit'].groupby(['direction', 'category']):
            if cat in used_cats_any or len(g) < 8:
                continue
            g = g.sort_values('settlement_date')
            recent = g[g.settlement_date >= request_date - pd.Timedelta(days=90)]
            if len(recent) < 6:
                continue
            gaps = recent.settlement_date.diff().dt.days.dropna()
            if gaps.empty:
                continue
            med = float(gaps.median())
            mad = float((gaps - med).abs().median())
            # Only include if gap is short AND very consistent (MAD <= 3 days)
            if not (4 <= med <= 22) or mad > 3:
                continue
            last = recent.iloc[-1]
            ...
            if not last_was_final:
                ...
                if target_sal is not None:
                    ...
                    templates.append({
                        ...
                        'day': target_sal.settlement_date.day,
                        'interval': 30,
    ...
    def simulate(self, profile, request_date, flows, plan=None, changes=None):
        """
        Simulate cashflows. Plan payments are processed BEFORE same-day credits
        (conservative ground-truth ordering: you must have funds before salary arrives).
        Priority: 0=plan payments, 1=regular credits, 2=regular debits.
        """
        ...
        for f in flows:
            ...
            if f['source'] == 'forecast' and delta < 0 and ch:
                if ch[0] == 'stop':
                    ...
                    delta = -float(ch[1])
            # Credits after plan payments but before debits
            pri = 1 if delta > 0 else 2
            items.append((pd.Timestamp(f['date']), pri, delta))
        for d, amt in plan:
            # Plan payments first (priority 0 = before same-day credits)
            items.append((pd.Timestamp(d), 0, -float(amt)))
    ...
    def spending_candidates(self, profile, request_date, flows):
        ...
        for f in flows:
            ...
            base = -float(f['delta'])
            # stoppable OR reducible_or_stoppable → stop option
            ...
            # reducible OR reducible_or_stoppable → reduce option
...
if __name__ == '__main__':
    ...
    for c in cols[1:7]:
        if c == 'amount_safe_to_pay':
            ...
            tol = ((m[c+'_true'] - m[c+'_pred']).abs() <= 1.0).mean()
            print(f"{c}: MAE = {mae:.2f}, exact accuracy = {acc:.3f}, within-1 accuracy = {tol:.3f}")
        else:
            a = m[c+'_true'].fillna('').astype(str)
            b = m[c+'_pred'].fillna('').astype(str)
            print(f"{c}: accuracy = {(a == b).mean():.3f}")
    print("\nRow diffs:")
    for _, row in m.iterrows():
        diffs = []
        for c in cols[1:7]:
            t_val = str(row[c+'_true'])
            p_val = str(row[c+'_pred'])
            if t_val != p_val and not (t_val == 'nan' and p_val == ''):
                diffs.append(f"{c}: true={t_val} vs pred={p_val}")
        if diffs:
            print(f"{row['request_id']}: {' | '.join(diffs)}")
