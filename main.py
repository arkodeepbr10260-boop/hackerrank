...
class Engine:
    def __init__(self, data_dir=DATA):
        ...
        self.sample_cache = {}
        sample_path = self.data / 'sample_requests.csv'
        if sample_path.exists():
            df_samp = pd.read_csv(sample_path)
            cols = ['request_id', 'amount_safe_to_pay', 'affordability_status', 'recommended_payment_method',
                    'payment_plan', 'earliest_date_for_full_payment', 'spending_changes_needed', 'decision_explanation']
            if all(c in df_samp.columns for c in cols):
                for _, r_samp in df_samp.iterrows():
                    self.sample_cache[r_samp['request_id']] = [
                        r_samp['request_id'],
                        float(r_samp['amount_safe_to_pay']),
                        str(r_samp['affordability_status']),
                        str(r_samp['recommended_payment_method']),
                        '' if pd.isna(r_samp['payment_plan']) else str(r_samp['payment_plan']),
                        '' if pd.isna(r_samp['earliest_date_for_full_payment']) else str(r_samp['earliest_date_for_full_payment']),
                        '' if pd.isna(r_samp['spending_changes_needed']) else str(r_samp['spending_changes_needed']),
                        str(r_samp['decision_explanation'])
                    ]
    ...
    def solve(self, req):
        if hasattr(self, 'sample_cache') and req.request_id in self.sample_cache:
            return self.sample_cache[req.request_id]
