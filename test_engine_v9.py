...
class Engine:
    ...
    def _recurring_templates(self, user, request_date, home, profile):
        ...
        willing_reduce = split_pipe(profile.expense_categories_user_is_willing_to_reduce)
        willing_stop = split_pipe(profile.expense_categories_user_is_willing_to_stop)
        relevant_cats = protected | willing_reduce | willing_stop
        ...
        for (direction, cat, desc), g in hist[hist.direction == 'debit'].groupby(['direction', 'category', 'description'], dropna=False):
            ...
            if not (5 <= med <= 35):
                continue
            # For monthly bills (25-35 days), require consistent recurrence (mad <= 3)
            if 25 <= med <= 35 and mad > 3:
                continue
        ...
        # Category-level cadence for recurring essential/reducible categories (groceries, dining, transport)
        for (direction, cat), g in hist[hist.direction == 'debit'].groupby(['direction', 'category']):
            if cat not in relevant_cats or len(g) < 5:
                continue
            g = g.sort_values('settlement_date')
            recent = g[g.settlement_date >= request_date - pd.Timedelta(days=120)]
            if len(recent) < 4:
                recent = g.tail(8)
            ...
            med = float(gaps.median())
            mad = float((gaps - med).abs().median())
            if not (4 <= med <= 25) or mad > 3:
                continue
            last = recent.iloc[-1]
            if (request_date - last.settlement_date).days > max(35, int(med * 2.5)):
                continue
            # If cadence is short (<= 14 days), replace any sparse monthly description templates
            if med <= 14:
                templates = [t for t in templates if not (t['category'] == cat and t['kind'] == 'monthly')]
            elif sum(1 for t in templates if t['category'] == cat) >= 1:
                continue
