...
class Engine:
    ...
    def _recurring_templates(self, user, request_date, home, profile):
        ...
        for (direction, cat), g in hist.groupby(['direction', 'category']):
            if direction != 'debit' or cat not in protected or (direction, cat) in exact_keys or len(g) < 8:
                continue
            g = g.sort_values('settlement_date')
            recent = g[g.settlement_date >= request_date - pd.Timedelta(days=60)]
            if len(recent) < 4:
                continue
        ...
        for t in templates:
            ...
            while d <= end:
                dup = any(f['source'] == 'explicit' and f['category'] == t['category'] and
                          ...
                          abs((f['date'] - d).days) <= 3 for f in flows)
                if not dup:
